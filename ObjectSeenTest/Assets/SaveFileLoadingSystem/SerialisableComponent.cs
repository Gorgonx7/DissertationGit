    ...
    public class SerialisableComponent<T> : ISerializableComponent where T : Component
    {
        ...
        public void addToGameObject(GameObject objectToAttatchTo)
        {
            ...
            foreach (KeyValuePair<String, ACE_PropertyField> i in componentVariable)
            {
                ...
                if (i.Value.m_type.Contains("UnityEngine"))
                {
                    string typeVal = i.Value.m_type;
                    if (typeVal == "UnityEngine.Mesh")
                    {
                        value = Resources.Load<Mesh>(i.Value.m_Val);
                    }
                }
                else
                {
                    ...
                    switch (reader.NodeType)
                    {
                        case XmlNodeType.Element:
                            if (reader.Name == componentType.ToString())
                            {
                                ...
                                {
                                    componentVariable.Add(reader.Name, new ACE_PropertyField(componentType.GetProperty(reader.Name).PropertyType.Name, reader.Value));
                                }
                            }
                            break;
